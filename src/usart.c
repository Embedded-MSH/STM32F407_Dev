...
void USART1_Init(uint32_t baudrate)
{
    ...
    printf("\r\r\r=========== STM32F407 外设初始报告 ===========\r");
...
void USART1_SendString(char* stringTemp)
{
    ...
    USART1_SendData((uint8_t*)stringTemp,
...
void USART2_Init(uint32_t baudrate)
{
    ...
    GPIO_Init(GPIOA,
              &GPIO_InitStructure);   // 初始化：把上述参数，更新到芯片寄存器
    // 配置RX_PA3
    ...
    GPIO_PinAFConfig(GPIOA,
                     GPIO_PinSource2,
                     GPIO_AF_USART2);   // 配置引脚复用功能：USART2
    GPIO_PinAFConfig(GPIOA,
                     GPIO_PinSource3,
                     GPIO_AF_USART2);   // 配置引脚复用功能：USART2
    ...
    USART_InitStructure.USART_Mode =
        USART_Mode_Rx | USART_Mode_Tx;          // 使能收、发模式
...
void USART2_IRQHandler(void)
{
    ...
    static uint8_t RxTemp
        [USART2_RX_BUF_SIZE];   // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断),
                                // 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；

    // 接收中断
    if (USART2->SR & (1 << 5))   // 检查RXNE(读数据寄存器非空标志位);
                                 // RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt
             >= USART2_RX_BUF_SIZE))   //||xUSART2.ReceivedFlag==1  // 判断1:
                                       // 当前帧已接收到的数据量，已满(缓存区),
                                       // 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            // 判断2:
            // 如果之前接收好的数据包还没处理，就放弃新数据，即，新数据帧不能覆盖旧数据帧，直至旧数据帧被处理．缺点：数据传输过快于处理速度时会掉包；好处：机制清晰，易于调试
            USART2
                ->DR;   // 读取数据寄存器的数据，但不保存．主要作用：读DR时自动清理接收中断标志；
            return;
        }
        RxTemp[cnt++] =
            USART2
                ->DR;   // 把新收到的字节数据，顺序存放到RXTemp数组中；注意：读取DR时自动清零中断位；
    }

    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (USART2->SR & (1 << 4))   // 检查IDLE(空闲中断标志位);
                                 // IDLE中断标志清理方法：序列清零，USART1 ->SR;
                                 // USART1 ->DR;
    {
        xUSART2.ReceivedNum = 0;   // 把接收到的数据字节数清0
        memcpy(
            xUSART2.ReceivedData,
            RxTemp,
            USART2_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中,
                                   // 等待处理;
                                   // 注意：复制的是整个数组，包括0值，以方便字符串数据
        xUSART2.ReceivedNum =
            cnt;   // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
        cnt = 0;   // 接收字节数累计器，清零; 准备下一次的接收
        memset(
            RxTemp,
            0,
            USART2_RX_BUF_SIZE);   // 接收数据缓存数组，清零; 准备下一次的接收
        USART2->SR;
    ...
    if ((USART2->SR & 1 << 7)
        && (USART2->CR1
            & 1 << 7))   // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        USART2->DR = usart2TxBuffer
            [usart2TxCounter++];   // 读取数据寄存器值；注意：读取DR时自动清零中断位；
        if (usart2TxCounter == usart2TxCount)
            USART2->CR1 &=
                ~(1 << 7);   // 已发送完成，关闭发送缓冲区空置中断 TXEIE
    }
}
 ...
 *          uint8_t  cnt      发送的字节数
 *，限于中断发送的缓存区大小，不能大于256个字节 返回值：
 ******************************************************************************/
void USART2_SendData(uint8_t* buf, uint16_t cnt)
{
    if (cnt > 256)
        cnt = 256;   // 本函数只适用于最大发送256个字节;
                     // 如果超过最大发送量，请自行分包发送，或者重写本函数

    for (uint16_t i = 0; i < cnt; i++)
        usart2TxBuffer[usart2TxCount++] = buf[i];

    if ((USART2->CR1 & 1 << 7)
        == 0)   // 检查发送缓冲区空置中断(TXEIE)是否已打开
        USART2->CR1 |= 1 << 7;
}
...
void USART2_SendString(char* stringTemp)
{
    ...
    USART2_SendData(
        (u8*)stringTemp,
        num);   // 注意调用函数所需要的真实数据长度;
                // 如果目标需要以0作结尾判断，需num+1：字符串以0结尾，即多发一个:0
}
...
void USART3_IRQHandler(void)
{
    static uint16_t cnt = 0;   // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t RxTemp
        [USART3_RX_BUF_SIZE];   // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断),
                                // 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；

    // 接收中断
    if (USART3->SR & (1 << 5))   // 检查RXNE(读数据寄存器非空标志位);
                                 // RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt
             >= USART3_RX_BUF_SIZE))   //||xUSART3.ReceivedFlag==1  // 判断1:
                                       // 当前帧已接收到的数据量，已满(缓存区),
                                       // 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            // 判断2:
    ...
    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (USART3->SR & (1 << 4))   // 检查IDLE(空闲中断标志位);
                                 // IDLE中断标志清理方法：序列清零，USART1 ->SR;
                                 // USART1 ->DR;
    {
        xUSART3.ReceivedNum = 0;   // 把接收到的数据字节数清0
        memcpy(
            xUSART3.ReceivedData,
            RxTemp,
            USART3_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中,
                                   // 等待处理;
                                   // 注意：复制的是整个数组，包括0值，以方便字符串数据
        xUSART3.ReceivedNum =
            cnt;   // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
        ...
        memset(
            RxTemp,
            0,
            USART3_RX_BUF_SIZE);   // 接收数据缓存数组，清零; 准备下一次的接收
        USART3->SR;
    ...
    if ((USART3->SR & 1 << 7)
        && (USART3->CR1
            & 1 << 7))   // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        USART3->DR = usart3TxBuffer
            [usart3TxCounter++];   // 读取数据寄存器值；注意：读取DR时自动清零中断位；
...
void UART4_IRQHandler(void)
{
    static uint16_t cnt = 0;   // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t RxTemp
        [UART4_RX_BUF_SIZE];   // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断),
                               // 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；

    // 接收中断
    if (UART4->SR & (1 << 5))   // 检查RXNE(读数据寄存器非空标志位);
                                // RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt
             >= UART4_RX_BUF_SIZE))   //||xUART4.ReceivedFlag==1   // 判断1:
                                      // 当前帧已接收到的数据量，已满(缓存区),
                                      // 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            // 判断2:
    ...
    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (UART4->SR & (1 << 4))   // 检查IDLE(空闲中断标志位);
                                // IDLE中断标志清理方法：序列清零，USART1 ->SR;
                                // USART1 ->DR;
    {
        ...
        memcpy(
            xUART4.ReceivedData,
            RxTemp,
            UART4_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中,
                                  // 等待处理;
                                  // 注意：复制的是整个数组，包括0值，以方便字符串数据
        xUART4.ReceivedNum =
            cnt;   // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
        ...
        memset(RxTemp,
               0,
               UART4_RX_BUF_SIZE);   // 接收数据缓存数组，清零; 准备下一次的接收
        UART4->SR;
    ...
    if ((UART4->SR & 1 << 7)
        && (UART4->CR1
            & 1 << 7))   // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        UART4->DR = uart4TxBuffer
            [uart4TxCounter++];   // 读取数据寄存器值；注意：读取DR时自动清零中断位；
...
void UART4_SendString(char* stringTemp)
{
    ...
    UART4_SendData(
        (u8*)stringTemp,
        num);   // 调用函数完成发送，num+1：字符串以0结尾，需多发一个:0
}
...
void UART5_IRQHandler(void)
{
    static uint16_t cnt = 0;   // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t RxTemp
        [UART5_RX_BUF_SIZE];   // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断),
                               // 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；

    // 接收中断
    if (UART5->SR & (1 << 5))   // 检查RXNE(读数据寄存器非空标志位);
                                // RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt
             >= UART5_RX_BUF_SIZE))   //||xUART5.ReceivedFlag==1   // 判断1:
                                      // 当前帧已接收到的数据量，已满(缓存区),
                                      // 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            // 判断2:
    ...
    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (UART5->SR & (1 << 4))   // 检查IDLE(空闲中断标志位);
                                // IDLE中断标志清理方法：序列清零，USART1 ->SR;
                                // USART1 ->DR;
    {
        ...
        memcpy(
            xUART5.ReceivedData,
            RxTemp,
            UART5_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中,
                                  // 等待处理;
                                  // 注意：复制的是整个数组，包括0值，以方便字符串数据
        xUART5.ReceivedNum =
            cnt;   // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
        ...
        memset(RxTemp,
               0,
               UART5_RX_BUF_SIZE);   // 接收数据缓存数组，清零; 准备下一次的接收
        UART5->SR;
    ...
    if ((UART5->SR & 1 << 7)
        && (UART5->CR1
            & 1 << 7))   // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        UART5->DR = uart5TxBuffer
            [uart5TxCounter++];   // 读取数据寄存器值；注意：读取DR时自动清零中断位；
...
void USART6_Init(uint32_t baudrate)
{
    ...
    GPIO_PinAFConfig(GPIOC,
                     GPIO_PinSource6,
                     GPIO_AF_USART6);   // 配置PC6复用功能：USART6
    GPIO_PinAFConfig(GPIOC,
                     GPIO_PinSource7,
                     GPIO_AF_USART6);   // 配置PC7复用功能：USART6
    ...
    printf("USART6初始化配置         接收中断、空闲中断, 发送中断\r");
}
...
void USART6_IRQHandler(void)
{
    static uint16_t cnt = 0;   // 接收字节数累计：每一帧数据已接收到的字节数
    static uint8_t RxTemp
        [USART6_RX_BUF_SIZE];   // 接收数据缓存数组：每新接收１个字节，先顺序存放到这里，当一帧接收完(发生空闲中断),
                                // 再转存到全局变量：xUSART.USARTxReceivedData[xx]中；

    // 接收中断
    if (USART6->SR & (1 << 5))   // 检查RXNE(读数据寄存器非空标志位);
                                 // RXNE中断清理方法：读DR时自动清理；
    {
        if ((cnt
             >= USART6_RX_BUF_SIZE))   //||(xUSART1.ReceivedFlag==1 // 判断1:
                                       // 当前帧已接收到的数据量，已满(缓存区),
                                       // 为避免溢出，本包后面接收到的数据直接舍弃．
        {
            // 判断2:
    ...
    // 空闲中断, 用于配合接收中断，以判断一帧数据的接收完成
    if (USART6->SR & (1 << 4))   // 检查IDLE(空闲中断标志位);
                                 // IDLE中断标志清理方法：序列清零，USART1 ->SR;
                                 // USART1 ->DR;
    {
        xUSART6.ReceivedNum = 0;   // 把接收到的数据字节数清0
        memcpy(
            xUSART6.ReceivedData,
            RxTemp,
            USART6_RX_BUF_SIZE);   // 把本帧接收到的数据，存放到全局变量xUSART.USARTxReceivedData中,
                                   // 等待处理;
                                   // 注意：复制的是整个数组，包括0值，以方便字符串数据
        xUSART6.ReceivedNum =
            cnt;   // 把接收到的字节数，存放到全局变量xUSART.USARTxReceivedCNT中；
        ...
        memset(
            RxTemp,
            0,
            USART6_RX_BUF_SIZE);   // 接收数据缓存数组，清零; 准备下一次的接收
        USART6->SR;
    ...
    if ((USART6->SR & 1 << 7)
        && (USART6->CR1
            & 1 << 7))   // 检查TXE(发送数据寄存器空)、TXEIE(发送缓冲区空中断使能)
    {
        USART6->DR = usart6TxBuffer
            [usart6TxCounter++];   // 读取数据寄存器值；注意：读取DR时自动清零中断位；
...
int fputc(int ch,
          FILE* f)   // 重定向fputc函数，使printf的输出，由fputc输出到UART,
                     // 这里使用串口1(USART1)
{
    HAL_UART_Transmit(&huart1, (uint8_t*)&ch, sizeof(ch), 5);
    return ch;
}
